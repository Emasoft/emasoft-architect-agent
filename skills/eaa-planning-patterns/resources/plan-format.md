...
SUFFICIENT: "Polling over webhooks | Third-party API has 30% webhook delivery failure in testing -> webhooks are unreliable, must fail fast if delivery fails -> polling provides deterministic delivery"
...
| ----------- | ------------- |
| Test Coverage Target | [Minimum % coverage required, e.g., "≥80% line coverage for core modules"] |
| Test Types Required | [Unit, Integration, E2E - specify which are mandatory] |
| Test Framework | [Framework/library to use, e.g., "pytest", "jest", "vitest"] |
| TDD Workflow | [RED-GREEN-REFACTOR enforcement: strict/recommended/optional] |
| Commit Message Pattern | [test/feat/refactor or alternative pattern] |

**TDD Phase Allocation**:
- RED Phase: [% of task time for test writing, typically 30-40%]
- GREEN Phase: [% of task time for minimal implementation, typically 40-50%]
- REFACTOR Phase: [% of task time for improvements, typically 10-30%]
...
**TDD Acceptance Criteria** (for code implementation milestones):

- ☐ Tests written before implementation code (RED phase complete)
