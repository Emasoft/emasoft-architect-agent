...
# WHY: Future annotations enable PEP 604 union syntax (X | Y) on Python 3.9
# and avoid forward reference issues with type hints
...
# WHY: Type alias for LSP config dicts enables mypy --strict compliance
# while keeping the config dict structure flexible for platform-specific keys
...
def is_binary_installed(binary: str) -> bool:
    ...
    # WHY: shutil.which() is the only reliable cross-platform method to check
    # for executables. subprocess.run(['which', binary]) fails on Windows.
...
def get_install_command(config: LSPConfig, plat: str) -> list[str] | None:
    ...
    if platform_key in config:
        return cast(list[str], config[platform_key])
    # Fall back to generic
    if "install_cmd" in config:
        return cast(list[str], config["install_cmd"])
    return None
...
def get_uninstall_command(config: LSPConfig, plat: str) -> list[str] | None:
    ...
    if platform_key in config:
        return cast(list[str] | None, config[platform_key])
    # Fall back to generic
    if "uninstall_cmd" in config:
        return cast(list[str] | None, config["uninstall_cmd"])
    return None
...
def get_lsp_state_file(project_path: Path | None = None) -> Path:
    ...
    """
    # WHY: Store state in .claude/ subdirectory to match Claude Code's conventions
    # and avoid polluting the project root with config files
    if project_path:
    ...
    if state_file.exists():
        try:
            with open(state_file, encoding="utf-8") as f:
                ...
                return cast(list[str], state.get("activated", []))
